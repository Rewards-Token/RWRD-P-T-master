...
jQuery(document).ready(function($) {
    ...
    async function init(){
        ...
        loadContract('./build/contracts/RWRDPToken.json', function(data){
            tokenContract = data;
            $('#tokenABI').text(JSON.stringify(data.abi));
        });
        loadContract('./build/contracts/RWRDPAirdrop.json', function(data){
    ...
    $('#executeAirdrop').click(function(){
        ...
        airdropLog.append('<div>Starting airdrop '+web3.utils.fromWei(airdropAmount) +' RWRDP to '+addresses.length+' addresses in batches of '+sendLimit+' addresses per transaction, starting from address '+sendStart+'.</div>');
